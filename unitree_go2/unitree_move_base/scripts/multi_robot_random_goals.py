...
class MultiRobotRandomGoalSender:
    ...
    def _detect_robots(self):
        """
        自动检测所有可用的机器人
        通过检查多个可能的 topic 来检测机器人：
        1. /robot_X/move_base_simple/goal (如果 move_base 已启动)
        2. /robot_X/odom (odometry topic)
        3. /robot_X/hokuyo/scan (laser scan topic)
        """
        robot_list = []
        ...
        if not robot_list:
            rospy.logwarn("未通过topic检测到机器人，尝试使用默认列表: robot_0, robot_1")
            default_robots = ['robot_0', 'robot_1', 'robot_2']
