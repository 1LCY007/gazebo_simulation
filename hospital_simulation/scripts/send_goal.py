...
# -*- coding: utf-8 -*-
"""
发送目标点给go2机器狗，使用move_base进行路径规划
使用方法:
    python3 send_goal.py x y yaw
    例如: python3 send_goal.py 5.0 3.0 0.0
...
class MoveBaseGoalSender:
    def __init__(self):
        ...
        # 创建action client
        self.client = SimpleActionClient('robot_0/move_base', MoveBaseAction)
        rospy.loginfo("等待move_base服务器连接...")
        
        # 等待服务器连接，最多等待5秒
        if not self.client.wait_for_server(rospy.Duration(5.0)):
            rospy.logerr("无法连接到move_base服务器！请确保move_base正在运行。")
            sys.exit(1)
    ...
    def print_planner_info(self):
        ...
        try:
            # 获取全局规划器
            global_planner = rospy.get_param('/robot_0/move_base/base_global_planner', 'navfn/NavfnROS')
            # 获取局部规划器
            local_planner = rospy.get_param('/robot_0/move_base/base_local_planner', 'teb_local_planner/TebLocalPlannerROS')
            ...
            if 'navfn' in global_planner:
                try:
                    use_dijkstra = rospy.get_param('/robot_0/move_base/NavfnROS/use_dijkstra', True)
                    use_quadratic = rospy.get_param('/robot_0/move_base/NavfnROS/use_quadratic', True)
                    print("Navfn配置:")
            ...
            if 'teb' in local_planner.lower():
                try:
                    max_vel_x = rospy.get_param('/robot_0/move_base/TebLocalPlannerROS/max_vel_x', 'N/A')
                    max_vel_theta = rospy.get_param('/robot_0/move_base/TebLocalPlannerROS/max_vel_theta', 'N/A')
                    print("TEB局部规划器配置:")
    ...
    def send_simple_goal(self, x, y, yaw=0.0, frame_id='map'):
        ...
        if x < -40 or x > 40 or y < -40 or y > 40:
            rospy.logwarn("警告: 目标点可能超出地图范围！地图范围约为 x: [-35, 35], y: [-35, 35]")
        pub = rospy.Publisher('/robot_0/move_base_simple/goal', PoseStamped, queue_size=1)
