...
class MoveBaseGoalSender:
    ...
    def print_planner_info(self):
        ...
        try:
            # 获取全局规划器
            global_planner = rospy.get_param('/move_base/base_global_planner', 'navfn/NavfnROS')
            # 获取局部规划器
            local_planner = rospy.get_param('/move_base/base_local_planner', 'teb_local_planner/TebLocalPlannerROS')
            ...
            if 'navfn' in global_planner:
                try:
                    use_dijkstra = rospy.get_param('/move_base/NavfnROS/use_dijkstra', True)
                    use_quadratic = rospy.get_param('/move_base/NavfnROS/use_quadratic', True)
                    print("Navfn配置:")
            ...
            if 'teb' in local_planner.lower():
                try:
                    max_vel_x = rospy.get_param('/move_base/TebLocalPlannerROS/max_vel_x', 'N/A')
                    max_vel_theta = rospy.get_param('/move_base/TebLocalPlannerROS/max_vel_theta', 'N/A')
                    print("TEB局部规划器配置:")
